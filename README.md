...
```
   ...
   ┌──────────▼───────────┐           ┌───────────▼──────────┐
   │       master1        │           │       master2        │
   │   192.168.88.119     │◄──etcd───►│   192.168.88.120     │
...
| 角色 | 主机名 | IP 示例 | 最低配置 |
|------|-------|---------|---------|
| Ansible 控制机 | — | 192.168.88.118 | 任意 Linux，安装 Ansible |
| 第一控制节点 | master1 | 192.168.88.119 | 4C 8G 50G |
| 第二控制节点 | master2 | 192.168.88.120 | 4C 8G 50G |
| 工作节点 | worker1～3 | 192.168.88.121～123 | 4C 8G 100G |
| 虚拟 IP | — | 192.168.88.200 | 同网段空闲 IP |

> **VIP 要求：** 必须是同网段内未被任何设备使用的空闲 IP，不需要提前配置，Keepalived 会自动绑定。
...
- **推荐：** CentOS Stream 9（已完整测试）
- **兼容：** RHEL 9、Rocky Linux 9、AlmaLinux 9
